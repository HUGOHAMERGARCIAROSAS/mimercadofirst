    ...
    <tr>
        ...
        <td>
            ...
            {{--<input type="text" class="form-control"--}}
                   {{--name="name"--}}
                   {{--onchange="updateColumn(this.value, '{{ $item->id }}', 'name', '{{ route('admin.category.updateInTable') }}')"--}}
{{--                   value="{{ $item->name }}"--}}
            {{-->--}}
        </td>
        <td>
            <img src="{{asset('movil/img/categoria/'.$item->imagen)}}" width="10%" alt="">
        </td>
        <td>
            <button
                    data-toggle="modal"
            ...
            <div class="modal fade" id="exampleModal_update_{{$item->id}}" tabindex="-1"
                ...
                <div class="modal-dialog modal-dialog-centered" role="document">
                    <div class="modal-content">
                        ...
                        <form action="{{ route('categories_prov.update', $item->id) }}"
            ...
            <button
                    data-url="{{ route('categories_prov.destroy', $item->id) }}"
                    data-type="delete"
